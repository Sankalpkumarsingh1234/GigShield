...
export default function AIChatAssistant({ userData, userId }) {
  const firstName = userData?.name?.split(" ")[0] || "there";
  const tierObj = TIERS.find((t) => t.id === (userData?.tier || "standard"));
  const [messages, setMessages] = useState([
    {
      role: "assistant",
      text: `Hi ${firstName}! 👋 I'm your GigShield AI assistant. Ask me anything about your coverage, premium, or disruption triggers.`,
    },
  ]);
  ...
  useEffect(() => {
    ...
    async function loadProfile() {
      try {
        ...
        console.error("Failed to load profile for AI context:", error);
      }
    }
  ...
  const systemPrompt = useMemo(() => {
    ...
    const dbPolicy = profile?.policy || {};
    const claims = profile?.claims || [];
    const claimSummary = claims.length
      ? `${claims.length} payouts so far totaling ₹${claims
          .reduce((sum, c) => sum + (c.amount || 0), 0)
          .toLocaleString()}`
      : "No previous payouts yet";

    return `You are GigShield's AI assistant — a friendly, concise insurance advisor for Indian food delivery workers (Zomato/Swiggy riders).

    
Worker context:
- Name: ${dbUser.name || userData?.name || "Unknown"}
- Platform: ${dbUser.platform || userData?.platform || "Zomato"}
- Zone: ${userData?.pinData?.zone || "Anna Nagar"}, ${userData?.pinData?.city || "Chennai"}
- NFI Risk Score: ${dbUser.nfi || userData?.nfi || 72}/100
- Plan: ${dbPolicy.tier || userData?.tier || "Standard"} plan at ₹${dbPolicy.premium || userData?.premium || 54}/week
- Max payout: ₹${dbPolicy.max_payout || tierObj?.max || 1000}/week
- Coverage: ${tierObj?.coverage?.join(", ") || "Rain, Flooding, AQI, Curfew"}
- Weekly earnings: ~₹${dbUser.earnings || userData?.earnings || 6000}
- Claim history: ${claimSummary}
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
        <div>
          ...
          <div style={{ fontSize: 11, color: "#9B9589" }}>
            Powered by Groq · Knows your policy
            {profile?.mock ? " (demo data)" : ""}
          </div>
        </div>
