...
export default function DashboardScreen({ data, onProfileUpdated, onPolicyCreated, onSignOut }) {
  const { name, platform, premium, tier, nfi, pinData, earnings, workerId, coverage } = data;
  const tierObj = TIERS.find((item) => item.id === tier) || TIERS[1];
  ...
  const [disruptionFeed, setDisruptionFeed] = useState([]);
  ...
  const fetchFeed = useCallback(async () => {
    try {
      ...
      setDisruptionFeed(feedData.feed || []);
      setFeedError(feedData.error || "");
    } catch {
      setDisruptionFeed([]);
      setFeedError("Could not load live disruptions right now.");
    } finally {
  ...
  const fetchTotalProtected = useCallback(async () => {
    try {
      ...
      setTotalProtected(claimsData.total || 0);
    } catch {
  ...
  function handleUPIComplete() {
    ...
    setTotalProtected((current) => current + payoutAmount);
    fetchTotalProtected();
  }
    ...
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
        <div>
          ...
          <div style={{ fontSize: 11, color: "#6B6258", marginTop: 1 }}>
            {platform} · {pinData.zone}, {pinData.city}
          </div>
        </div>
        ...
        {TABS.map((tab) => (
          <button
            ...
            style={{
              ...
              borderRadius: 7,
              border: "none",
              background: activeTab === tab.id ? "#1A1512" : "#F5F0EB",
              color: activeTab === tab.id ? "#fff" : "#6B6258",
              fontSize: 10,
              fontWeight: 600,
              cursor: "pointer",
              whiteSpace: "nowrap",
      ...
      {activeTab === "dashboard" && (
        <>
          {stormAlert && (
            <div style={{ padding: "11px 13px", background: "linear-gradient(135deg,#FFF3CD,#FFE4A0)", border: "1.5px solid #F59E0B", borderRadius: 12, marginBottom: 12, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <div style={{ display: "flex", gap: 6 }}>
                <button onClick={() => setStormAlert(false)} style={{ padding: "5px 10px", borderRadius: 8, border: "1px solid #D97706", background: "#F59E0B", color: "#fff", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>+Rs8</button>
                <button onClick={() => setStormAlert(false)} style={{ padding: "4px 7px", borderRadius: 8, border: "1px solid #E0D9D0", background: "transparent", color: "#9B9589", fontSize: 11, cursor: "pointer" }}>X</button>
              </div>
            </div>
          )}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 12 }}>
            {[
              { label: "Protected", value: `Rs${Math.round(totalProtected).toLocaleString()}`, sub: "this month", color: "#4CAF82" },
              { label: "Premium", value: `Rs${premium}`, sub: tierObj?.name || "Standard", color: "#FF6B35" },
              { label: "NFI score", value: nfi, sub: nfi > 65 ? "High risk" : nfi > 40 ? "Moderate" : "Low risk", color: nfi > 65 ? "#EF4444" : "#F59E0B" },
            ].map((stat, index) => (
              <div key={index} style={{ padding: "10px 8px", background: "#FAFAF8", border: "1px solid #E0D9D0", borderRadius: 12, textAlign: "center" }}>
                <div style={{ fontFamily: "serif", fontSize: 17, color: stat.color }}>{stat.value}</div>
                <div style={{ fontSize: 9, fontWeight: 600, color: "#1A1512", marginTop: 2 }}>{stat.label}</div>
                <div style={{ fontSize: 9, color: "#9B9589" }}>{stat.sub}</div>
              </div>
            ))}
          </div>

          <div style={{ display: "flex", flexWrap: "wrap", gap: 4, marginBottom: 12 }}>
            {(coverage || tierObj?.coverage || []).map((item, index) => (
              <span key={index} style={{ fontSize: 10, padding: "2px 8px", background: "#E8F5EE", color: "#2D6B4A", borderRadius: 20, fontWeight: 600 }}>
                {item}
              </span>
            ))}
          </div>
          ...
          {payout && !showUPI && (
            <div style={{ padding: "14px", background: "#E8F5EE", border: "2px solid #4CAF82", borderRadius: 14, marginBottom: 12, animation: "slideIn 0.4s ease" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                ...
                <div style={{ fontFamily: "serif", fontSize: 26, color: "#2D6B4A" }}>Rs{payout.amount}</div>
              </div>
            </div>
          )}
          ...
          {disruptionFeed.length ? (
            <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
              {disruptionFeed.slice(0, 6).map((item, index) => (
                <div key={item.id || index} style={{ display: "flex", gap: 10, padding: "9px 11px", background: "#FAFAF8", border: "1px solid #E0D9D0", borderRadius: 10, alignItems: "center" }}>
            ...
            <div style={{ padding: "12px 14px", background: "#FAFAF8", border: "1px solid #E0D9D0", borderRadius: 10 }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: "#1A1512" }}>No live disruptions right now</div>
              <div style={{ fontSize: 10, color: "#6B6258", marginTop: 4 }}>
                {feedError || "This feed now shows only real disruptions stored in the database."}
              </div>
            </div>
          )}
        </>
      ...
      {activeTab === "policy" && <PolicyReceipt data={data} onPolicyCreated={onPolicyCreated} />}
      ...
      {activeTab === "profile" && (
        <form onSubmit={saveProfile} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          {[
            ...
            { key: "earnings", label: "Weekly earnings (Rs)", type: "number", placeholder: "e.g. 6000" },
          ].map((field) => (
            <label key={field.key} style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  if (field.key === "pinCode") value = value.replace(/\D/g, "");
                  setProfileForm((form) => ({ ...form, [field.key]: value }));
                }}
          ...
          <label style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              {["Zomato", "Swiggy"].map((item) => (
                <button
                  key={item}
                  type="button"
                  onClick={() => setProfileForm((form) => ({ ...form, platform: item }))}
                  style={{ flex: 1, padding: "10px", borderRadius: 10, border: "1.5px solid", borderColor: profileForm.platform === item ? "#FF6B35" : "#E0D9D0", background: profileForm.platform === item ? "#FFF0EB" : "#FAFAF8", color: profileForm.platform === item ? "#FF6B35" : "#6B6258", fontWeight: 600, fontSize: 14, cursor: "pointer" }}
                >
                  {item}
                </button>
              ))}
          ...
          {profileStatus && (
            <div style={{ padding: "10px 12px", background: profileStatus.includes("successfully") ? "#E8F5EE" : "#FEE2E2", borderRadius: 10, fontSize: 12, color: profileStatus.includes("successfully") ? "#2D6B4A" : "#991B1B" }}>
              {profileStatus}
            </div>
          )}
